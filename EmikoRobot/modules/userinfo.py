...
def info(update: Update, context: CallbackContext):
    ...
    rep = message.reply_text("<code>Getting info from 亗 ᏆᏀΝᏆͲᎬ...</code>", parse_mode=ParseMode.HTML)
    ...
    if user_id not in [bot.id, 777000, 1087968824]:
        ...
        text += f"┏╾\n\n<b>𝙷𝙴𝙰𝙻𝚃𝙷:</b> </code> </i>{userhp['percentage']}% \n[<b>{make_bar(int(userhp['percentage']))}]"
    ...
    try:
        ...
        if spamwtc:
            ...
            text += "\nAppeal at @naruto_xRobot_Support"
    except:
    ...
    if user.id == OWNER_ID:
        ...
        text += "\n\n\[ 𝙿𝙾𝚆𝙴𝚁𝚂 ]."
        text += "\n\nThe user is a member of '亗ᏆᏀΝᏆͲᎬ'."
              
        text += "\n\nThe user is a 'Chunin'."
        disaster_level_present = True
        ...
        text += (
            "\n\nOwner Of A Bot. Queen Of @excrybaby. Bot Name Inspired From 'Naruto'."
        )
        disaster_level_present = True
            ...
            message.reply_document(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "Health", url="https://t.me/naruto_blah/10"
                            ),
                            InlineKeyboardButton(
                                "Disaster", url="https://t.me/naruto_blah/5"
                            ),
                        ],
                    ]
                ),
...
def stats(update: Update, context: CallbackContext):
    stats = "<b> 卐 Current Naruto Statistics 卐 </b>\n" + "\n".join(

        [mod.__stats__() for mod in STATS]
    )
    result = re.sub(r"(\d+)", r"<code>\1</code>", stats)

    result += "\n<b> 卐 Powered By Rin Okumura 卐 </b>"
    update.effective_message.reply_text(
...
*What is that health thingy?*
 Come and see [HP System explained](https://t.me/naruto_blah/10)
"""
